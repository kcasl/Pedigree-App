...
export function createViewTemplate(
  ...
  out[slots.ggf] = person(slots.ggf, names.ggf, createdAt, 'male', { spouseId: slots.ggm });
  out[slots.ggm] = person(slots.ggm, names.ggm, createdAt, 'female', { spouseId: slots.ggf });
  ...
  const sibFather = view === 'self' ? slots.father : slots.gf;
  const sibMother = view === 'self' ? slots.mother : slots.gm;
