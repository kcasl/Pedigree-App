...
import { scaleSize, clampFontScale } from '../theme/responsive';
...
function childConnectY(node: PositionedNode, cfg: EdgeDrawConfig): number {
  return top(node) - cfg.childConnectInset;
}
...
function drawCouple(group: CoupleGroup, ctx: Ctx): void {
  ...
  const minChildConnect = Math.min(...children.map(ch => childConnectY(ch, cfg)));
  const ry = railCenterY(lineCenterY, minChildConnect);
  const join = stroke / 2;
